...
public class PersonService {
    ...
    public synchronized static void personInfo(JdbcTemplate jdbcTemplate) {
...
//        sqlList.add("a.`gz`,a.`fjh`,a.`mark`,c.`MC` AS gzmc,e.sn,b.`dwid`,");
        sqlList.add("a.reason,a.reasonStatus,");
        ...
        List<Integer> tyIdList = new ArrayList<>();
        List<Integer> mkIdList = new ArrayList<>();
...
//                    row.put("reason",rs.getInt("reason"));
                    ...
                    int reasonStatus = rs.getInt("reasonStatus");
                    ...
                    if (sendzt == 1) {
                        ...
                        else if (sendzt == 2 && !Objects.isNull(rs.getInt("loss_id")) && rs.getInt("loss_id") != 0 && mark ==1) {
                            row.put("rykStatus", "3");
                            mkIdList.add(rs.getInt("sn"));
                        }// 停用卡：t_ry_jbxx中reason字段为2标识为已停用
                        else if (reasonStatus == 2 && reason == 2 && sendztbd==1) {
                            row.put("rykStatus", "4");
                            tyIdList.add(rs.getInt("id"));
                            // 重新启用卡：t_ry_jbxx中reason字段为2且reasonStatus=2
                        } else if (reason == 0 && reasonStatus == 2 && sendztbd==2) {
                            row.put("rykStatus", "5");
                            qyIdList.add(rs.getInt("id"));
                        }
                    }
...
//                String url = "http://172.16.1.191:8080/park/api/getPersonList";
                String url = "http://172.16.1.191:8083/craftsman-admin/api/getPersonList";
                ...
                if(!mkIdList.isEmpty()){
                    jdbcTemplate.update("update t_ry_jbxx set mark=2 where card_sn in(" + mkIdList.stream().map(f -> String.valueOf(f)).collect(Collectors.joining(",")) + ")");
                }
                if(!qyIdList.isEmpty()){
                    jdbcTemplate.update("update t_ry_jbxx set reasonStatus=1 where id in(" + qyIdList.stream().map(f -> String.valueOf(f)).collect(Collectors.joining(",")) + ")");
                }
                if(!tyIdList.isEmpty()){
                    jdbcTemplate.update("update t_ry_jbxx set sendztbd=2 where id in(" + tyIdList.stream().map(f -> String.valueOf(f)).collect(Collectors.joining(",")) + ")");
                }
                if (!jbIdList.isEmpty()) {
                    jdbcTemplate.update("update t_ry_jbxx set sendztjb=2,status=0,mc=card_sn where id in(" + jbIdList.stream().map(f -> String.valueOf(f)).collect(Collectors.joining(",")) + ")");
                    jdbcTemplate.update("update t_ry_jbxx set card_sn='',gh='',zz='',sjh1='',uuid='' where id in(" + jbIdList.stream().map(f -> String.valueOf(f)).collect(Collectors.joining(",")) + ")");
                    //jdbcTemplate.update("delete * from t_ryk_jbxx where ryid in(" + jbIdList.stream().map(f -> String.valueOf(f)).collect(Collectors.joining(",")) + ")");
                }
