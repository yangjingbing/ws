...
//                listSql = "SELECT cardid, v FROM t_rssi WHERE cardid in ('" + cardIdList.stream().collect(Collectors.joining("','")) + "')";
                ...
                try {
                    ...
                    jdbcTemplate.update("update t_car_scj set sendzt=2 where description ='" + tmpMap.getOrDefault("description","").toString() + "' and carnum like '%" + tmpMap.getOrDefault("carnum", "").toString().trim().replace(" ","") + "%'");
