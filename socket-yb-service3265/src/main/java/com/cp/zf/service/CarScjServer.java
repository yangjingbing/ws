...
/**
 * @Description 手持机上传通行记录 .</br>
 * <></>
 * @Author gu
 * @Date 2021/1/26 11:16
 * @Version 1.0.0
 **/
public class CarScjServer {

    static Pattern r = Pattern.compile("\\d+");

//    private static Logger log = Logger.getLogger(CarScjServer.class.getClass());


    public static void carscjInfo(JdbcTemplate jdbcTemplate, long lastt) {
...
//        Timestamp tt = Timestamp.valueOf(nowTime);
        ...
        String listSql = "SELECT carnum,COUNT(1) n,description,dt,sip,pasted FROM t_car_scj t WHERE sendzt=1 GROUP BY t.carnum,t.description";
        List<Map<String, Object>> result = jdbcTemplate.query(listSql, new RowMapper() {
            public Map mapRow(ResultSet rs, int rowNum)
                ...
                try {
                    ...
                    if (!StringUtils.isEmpty(rs.getString("carnum"))) {
        ...
        System.out.println("当前SCJ查询结果：" + JSON.toJSONString(result));
//        log.info("当前SCJ查询结果：" + JSON.toJSONString(result));
        if (!result.isEmpty()) {
            ...
            for (Iterator<Map<String, Object>> iterator = result.iterator(); iterator.hasNext(); ) {
                ...
                System.out.println("获取最终的scj sn:" + cardIdList.stream().collect(Collectors.joining("','")));
                //t_ryk_jbxx
                ...
                //t_rssi
                listSql = "SELECT cardid, v FROM t_rssi WHERE cardid in ('" + cardIdList.stream().collect(Collectors.joining("','")) + "') and (v != 0 or v is not null) group by cardid";
                List<Voltage> voltageResult = jdbcTemplate.query(listSql, new RowMapper() {
                    public Voltage mapRow(ResultSet rs, int rowNum)
                            throws SQLException {
                        Voltage voltage = new Voltage();
                        try {
                            BigDecimal v = new BigDecimal(rs.getInt("v")).divide(new BigDecimal(10));
                            voltage.setNewV(v.setScale(2, RoundingMode.HALF_DOWN).stripTrailingZeros().toPlainString());
                ...
                //电压值
//                Map<Integer, String> voltageMap = new HashMap<>();
//                if (!voltageResult.isEmpty()) {
//                    voltageMap = voltageResult.stream().collect(Collectors.toMap(Voltage::getCardid, Voltage::getV, (x1, x2) -> x1));
//                }
                Map<Integer, String> voltageMap = new HashMap<>();
                if (!voltageResult.isEmpty()) {
                    voltageMap = voltageResult.stream().collect(Collectors.toMap(Voltage::getCardid, Voltage::getNewV, (x1, x2) -> x1));
                }
                ...
                sendResultMap.put("Reason", tmpMap.get("description"));
                ...
                if (!CollectionUtils.isEmpty(personList)) {
                    ...
                    for (Map<String, Object> info : personList) {
                        ...
                        //personCardMap.put("Volit", voltageMap.getOrDefault(info.get("sn"), "3.6"));
                        personCardMap.put("Volit", 3.6);
                ...
                try {
                    System.out.println("手持机核验通行记录数据。");
//                    HttpClient.httpPost("http://172.16.1.191:8080/park/api/accept/pass/hand", resultMap, null);
                    HttpClient.httpPost("http://172.16.1.191:8083/craftsman-admin/api/accept/pass/hand", resultMap, null);
                    startTime = System.currentTimeMillis();

                    String updateSQL = "update t_car_scj set sendzt=2 where description ='" + tmpMap.getOrDefault("description", "").toString() + "' and carnum like '%" + tmpMap.getOrDefault("carnum", "").toString().trim().replace(" ", "") + "%'";
                    int count = jdbcTemplate.update(updateSQL);
                    System.out.println(String.format("t_car_scj 修改%s 耗时：%s count：%s sql:%s", count > 0 ? "成功" : "失败", (System.currentTimeMillis() - startTime), count, updateSQL));
            ...
            System.out.println("手持机无核验通行记录数据");
//            log.info("手持机无核验通行记录数据。");
        }
    }
