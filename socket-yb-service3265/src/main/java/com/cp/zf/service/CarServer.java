...
//    private static Logger log = Logger.getLogger(CarServer.class.getClass());
    ...
    public synchronized static void carInfo(JdbcTemplate jdbcTemplate, long lastt) {
        ...
        List<Map<String, Object>> result = jdbcTemplate.query(listSql, new Object[]{tt}, new RowMapper() {
            public Map mapRow(ResultSet rs, int rowNum)
                ...
                try {
                    ...
                    //获取sn(人员卡卡号)
                    row.put("sn", "");
//                    log.info("当前description：" + rs.getString("description"));
...
//                        log.info("sn before:"+StringUtil.join(snList, ","));
...
//        log.info("car query result:"+(System.currentTimeMillis()-startTime));
        startTime = System.currentTimeMillis();
        System.out.println("car query result："+ JSON.toJSONString(result));
        if (!result.isEmpty()) {
...
//                log.info(String.format("获取最终的sn：%s" , cardIdList.stream().collect(Collectors.joining("','"))));
                //t_ryk_jbxx
...
//                            double v = rs.getDoubl
//                            BigDecimal v = new BigDecimal(rs.getInt("v"));
//                            voltage.setNewV();
...
//                        personCardMap.put("Volit", voltageMap.getOrDefault(info.get("sn"), "3.6"));
                ...
                try {
//                    log.info("存在通行记录数据，准备调用http服务。");
//                    HttpClient.httpPost("http://172.16.1.191:8080/park/api/accept/pass", resultMap, null);
                    HttpClient.httpPost("http://172.16.1.191:8083/craftsman-admin/api/accept/pass", resultMap, null);
//                    log.info("httpclient 耗时："+(System.currentTimeMillis()-startTime));
                    startTime = System.currentTimeMillis();
                    String updateSQL = "update t_car set sendzt=2 where description ='" + tmpMap.getOrDefault("description","").toString() + "' and carnum like '%" + tmpMap.getOrDefault("carnum", "").toString().trim().replace(" ","") + "%'";
                    int count = jdbcTemplate.update(updateSQL);
                    String outStrTmp = String.format("t_car 修改%s 耗时：%s count：%s sql:%s", count > 0 ? "成功" : "失败", (System.currentTimeMillis() - startTime), count, updateSQL);
                    System.out.println(outStrTmp);
//                    log.info(outStrTmp);
